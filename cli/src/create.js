...
export async function create(gistIdOrOptions, options = {}) {
  console.log('');
  console.log(chalk.dim('  vibeship spawner'));
...
async function createFromGist(gistId, options) {
  ...
  try {
    ...
    if (!preflightOk) {
      ...
      console.log(chalk.dim('  Run `npx vibeship-spawner doctor` for more details.'));
      console.log('');
      process.exit(1);
    }
...
async function createInteractive(options) {
  ...
  if (selectedTemplate.openBrowser) {
    ...
    console.log(chalk.cyan('  npx vibeship-spawner create <gist-id>'));
    console.log('');
  ...
  console.log('');
  console.log(chalk.dim(`  Your agents: ${selectedAgents.join(', ')}`));
