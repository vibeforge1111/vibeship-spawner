 ...
 */
export function renderAgentLane(agent: AgentState): string {
  const lines: string[] = [];
  ...
  if (agent.status === 'waiting' && agent.waitingFor) {
    lines.push('');
    lines.push(colorize(
      `${ICONS.waiting} Waiting: ${agent.waitingFor}`,
      COLORS.warning
    ));
  ...
  if (agent.status === 'error' && agent.error) {
    lines.push('');
    lines.push(colorize(
      `${ICONS.warning} ${truncate(agent.error, 45)}`,
      COLORS.error
    ));
  }
 ...
 */
export function renderAgentLaneMinimal(agent: AgentState): string {
  const icon = agent.icon || getAgentIcon(agent.name);
  ...
  return colorize(
    `${icon} ${agent.name} ${statusIcon}${durationStr}`,
    statusColor
  );
}
